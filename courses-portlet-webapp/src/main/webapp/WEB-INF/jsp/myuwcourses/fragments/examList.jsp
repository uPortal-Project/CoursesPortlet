...
<c:forEach items="${ finalExamsList }" var="finalExam">
  ...
  ${finalExam.courseDescr}&nbsp;${finalExam.courseCode}: &nbsp;${finalExam.courseTitle}<br/>
  ${finalExam.meetingDescr}<br/>
  <c:if test="${!empty finalExam.room}">${finalExam.room}&nbsp;</c:if>${finalExam.locationDisplayName}<br/>
  ${finalExam.timeRangeDisplay}<br/><br/>
</c:forEach>